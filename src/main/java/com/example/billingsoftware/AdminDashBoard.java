...
public class AdminDashBoard implements Initializable {
    ...
    public Button exportProductListButton;
    public Button exportManageStaffButton;

    // ... (your existing code)

    public void exportProductListButtonListener(ActionEvent actionEvent) {
        exportToCSV(productTable.getItems(), "ProductList.csv");
    }

    public void exportManageStaffButtonListener(ActionEvent actionEvent) {
        exportToCSV(userTable.getItems(), "ManageStaff.csv");
    }

    private <T> void exportToCSV(List<T> data, String filename) {
        File file = new File(filename);

        try (CSVWriter writer = new CSVWriter(new FileWriter(file))) {
            String[] headers = getObjectFields(data.get(0));
            writer.writeNext(headers);

            for (T obj : data) {
                String[] rowData = getObjectValues(obj);
            ...
            alert.setContentText("Data exported to " + filename);
            alert.show();
        } catch (IOException e) {
            e.printStackTrace();
            ...
            alert.setHeaderText(null);
            alert.setContentText("Error exporting data to " + filename);
            alert.show();
        }
    }

    private <T> String[] getObjectFields(T obj) {
        // Assuming the fields are String, you might need to adjust for other types
        // or customize as per your data model
        // This example assumes that your data model class has getters for all fields
        // and the fields are of type String
        return Arrays.stream(obj.getClass().getDeclaredFields())
                .map(Field::getName)
                .toArray(String[]::new);
    }

    private <T> String[] getObjectValues(T obj) {
        // Assuming the fields are String, you might need to adjust for other types
        // or customize as per your data model
        // This example assumes that your data model class has getters for all fields
        // and the fields are of type String
